...
def test_the_last_season_frame_carries_what_the_card_shows():
    ...
    `_last_season_stats` moved from the Draft page into `ui/player_card.py`
    (the player card extraction), so that is where this now reads from.
    """
    import re
    from pathlib import Path
    src = Path("ui/player_card.py").read_text()
